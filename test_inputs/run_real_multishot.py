...
from video_pipeline.multishot import run_multishot as rm

num_shots   = int(sys.argv[1]) if len(sys.argv) > 1 else 8
shot_seconds= int(sys.argv[2]) if len(sys.argv) > 2 else 5
gender      = (sys.argv[3].lower() if len(sys.argv) > 3 else "f")
run_dir = Path("/workspace/alluvi-pipeline/outputs")/datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S_multishot_real")
...
path = rm.build_one(account, output, num_shots, 12345, run_dir, handles, target_seconds=shot_seconds)
print("\nDONE multishot ->", path)
